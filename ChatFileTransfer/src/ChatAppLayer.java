...
public class ChatAppLayer implements BaseLayer {
    ...
    /**
     * 
     * ChatFileTransfer 과제에서는 simplest로 구현하였다.
     * 단순히 이전 과제인 stop&wait에서 send와 fragSend 함수에서 
     * waitACK만을 제거하여 구현할 수 있다.
     * 
     * */
    ...
    public synchronized boolean Receive(byte[] input) {
        ...
        if(tempType == 0) {												//타입이 0일때, 단편화 되지 않고 한번에 받아온 프레임이다. 
            data = RemoveCappHeader(input, input.length);				//헤더를 제거하고 
            this.GetUpperLayer(0).Receive(data);						//윗 레이어로 전달 
        }
        else{
        	if(tempType == 1) {
        		int size = byte2ToInt(input[0], input[1]);				//헤더의 0번, 1번 인덱스, 크기
        		fragBytes = new byte[size];								//단편화된 크기만큼의 fragBytes배열을 만들어준다.
        		fragCount = 1;
        		tempBytes = RemoveCappHeader(input, input.length);		//헤더를 제거한것을 tempbytes로 저장하고
        		...
        		System.arraycopy(tempBytes, 0, fragBytes, (fragCount++) * 1456, byte2ToInt(input[0], input[1]));		//fragbytes의 알맞은 위치에 tempbytes를 추가
        																												//이 부분은 중간부분 단편화 부터 마지막 부분 단편화를 담당
        		if(tempType == 3) {
